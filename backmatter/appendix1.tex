...
Le sezioni tematiche sono due, una relativa all'utilizzo delle principali funzionalità offerte dal \pacchettor{} e una relativa all'utilizzo di \acsfont{Sensors} \acs{DLL} per la generazione di dataset da modelli \acs{TSIS}.

Tutti gli esempi presentati si riferiscono alla variante estesa, con elementi da $100$ secondi, del dataset relativo a \emph{Viale Cesare Battisti} (\ie{} \ds{2\text{.}E\text{.}100}). In questo capitolo, per brevità, ci si riferirà a tale dataset semplicemente con il termine \ds{2}.
...
A questo punto è possibile procedere all'installazione del \pacchettor{}. Poiché esso non è ancora stato pubblicato nell'archivio online dei pacchetti \lstinline$R$\footnote{Una delle modalità di installazione di pacchetti \lstinline$R$ è tramite connessione a un archivio online in cui essi vengono memorizzati, il \acs{CRAN}.}, la procedura di installazione differisce da quella precedentemente mostrata. A tale scopo si utilizza quindi il pacchetto \lstinline$devtools$ per installare \rctbn{}. Si osservi che il pacchetto \lstinline$devtools$ permette di creare e installare un pacchetto \lstinline$R$ a partire dal suo sorgente (purché esso rispetti le dovute convenzioni e contenga codice eseguibile senza errori), che sia esso online o sul sistema dell'utente, o da un archivio compresso. Perciò, disponendo del sorgente di \rctbn{}, è possibile installarlo come pacchetto \lstinline$R$ digitando le seguenti istruzioni.
...
\vspace*{8pt}\inputsourcecode[caption={[Importazione e serializzazione (\ds{2})]Importazione e serializzazione del \ds{2}.},language=rstats,label=lst:rctbn-import-ds]{codes/importctbnds}
...
\vspace*{8pt}\inputsourcecode[caption={[Caricamento dataset (\ds{2})]Caricamento del \ds{2}.},language=rstats,label=lst:rctbn-load-ds]{codes/loadctbnds}
...
In questa sottosezione si illustrano le funzionalità che \rctbn{} fornisce affinché l'utente possa computare le \emph{\stats{}}\index{statistiche sufficienti}.
...
\vspace*{8pt}\inputsourcecode[caption={[Calcolo delle \stats{} (\ds{2})]Esempio di calcolo delle \emph{\stats{}} sul \ds{2}.},language=rstats,label=lst:rctbn-fsstats]{codes/fsstats}
...
\vspace*{8pt}\inputsourcecode[language=rstats,caption={[Calcolo delle \stats{} (\ds{2})]Esempio di calcolo delle \emph{\stats{}} di più nodi dato un insieme di genitori comune, il nodo classe (\ds{2}).},label=lst:rctbn-fsstats2]{codes/fsstats2}
...
Si osservi che la funzione \lstinline[language=rstats]{fsstats} permette anche di calcolare le \emph{\stats{}}\index{statistiche sufficienti} di uno o più nodi senza condizionare la loro computazione ad alcun genitore.

Di seguito viene illustrato come effettuare tale operazione.

\vspace*{8pt}\inputsourcecode[language=rstats,caption={[Calcolo delle \stats{} (\ds{2})]Esempio di calcolo delle \emph{\stats{}} di un nodo senza condizionare la computazione a un insieme di genitori (\ds{2}).},label=lst:rctbn-fsstats3]{codes/fsstats3}
...
\vspace*{8pt}\inputsourcecode[language=rstats,caption={[Calcolo delle \stats{} (\ds{2})]Esempio di calcolo delle \emph{\stats{}} di più nodi aventi diversi insiemi di nodi genitori (\ds{2}).},label=lst:rctbn-ffstatsgraph]{codes/fsstatsgraph}
...
Si illustra ora come effettuare l'apprendimento dei parametri di una \acs{CTBN} da un insieme di \emph{dati completi}, nello specifico dal \ds{2}, tramite la relativa funzionalità offerta dal \pacchettor{}. Si ricorda, come discusso nella \autoref{subsec:ctbn-params} \vpageref{subsec:ctbn-params}, che è possibile effettuare la stima dei parametri\index{stima!parametri} tramite due approcci: la stima \acl{MLE}\index{stima!\acl{MLE}} o la stima bayesiana\index{stima!bayesiana}.
...
\inputsourcecode[caption={[Stima dei parametri di una \acs{CTBN} (\ds{2})]Apprendimento (\ie{} stima esatta) dei parametri di una \acs{CTBN} dal \ds{2}.},language=rstats,label=lst:rctbn-hparams]{codes/params}
...
Per controllare il modo in cui i parametri vengono computati, cioè se essi vengono appresi tramite stima \acl{MLE} o tramite stima bayesiana, il \pacchettor{} fornisce $3$ opzioni, presentate di seguito.
\begin{itemize}
...
\end{itemize}
Perciò, l'esempio presentato dal sorgente \ref{lst:rctbn-hparams} effettua la \emph{stima bayesiana\index{stima!bayesiana}} dei parametri, calcolando $\paramhat{q}$ e $\greekhat{\theta}$.
...
\vspace*{8pt}\inputsourcecode[caption={[Calcolo delle \acs{CIM} di una \acs{CTBN} (\ds{2})]Calcolo delle \acs{CIM} della \acs{CTBN} d'esempio dal \ds{2}.},language=rstats,label=lst:rctbn-cim-comput]{codes/cims}
...
In questa sottosezione si presenta l'utilizzo del \pacchettor{} per l'apprendimento\index{apprendimento!\acsfont{CTBNC}} di un classificatore\index{apprendimento!classificatore} \acs{CTBN} (\acs{CTBNC}).

A tale scopo si presenta una porzione di codice \lstinline$R$ che, generato in modo casuale un classificatore \acs{CTBN} valido (si veda la \autoref{defn:ctbnc} nella \vref{sec:ctbnc-model}) per il \ds{2}, utilizza la funzione \lstinline[language=rstats]{learn_ctbnc} per apprenderne i parametri e la distribuzione a priori delle classi rispetto a un \emph{training set} di tale dataset.
...
Di seguito si illustra il codice sorgente che, utilizzando tale funzione (si veda la \autoref{lst:learn-ctbnc-func-line}), apprende\index{apprendimento!\acsfont{CTNBC}} un \acs{CTBNC} da un \emph{training set} del \ds{2} (\ie{} da una porzione di elementi di tale dataset).
Si noti che prima d'eseguire tale operazione, da \autoref{lst:gen-valid-ctbnc-start} a \autoref{lst:gen-valid-ctbnc-end}, viene generata una struttura valida e casuale per un classificatore \acs{CTBN}; a \autoref{lst:gen-fake-ts} viene invece generato l'ipotetico \emph{training set}.

\vspace*{8pt}\inputsourcecode[caption={[Apprendimento di un \acs{CTBNC} (\ds{2})]Apprendimento di un classificatore \acs{CTBN}, la cui struttura viene generata in modo casuale, su un \emph{training set} facente parte del \ds{2}.},language=rstats,label=lst:rctbn-learc-ctbnc]{codes/learnctbnc}
...
\subsection{Classificazione\index{classificazione!\acsfont{CTNBC}} di un CTNBC}
Appreso un classificatore \acs{CTBN} su un \emph{\keyword{training set}} è chiaramente possibile, come discusso in questo lavoro di tesi, utilizzarlo per compiere la classificazione\index{classificazione!supervisionata} di una qualsiasi istanza del \emph{\keyword{test set}}. A tale scopo si necessita di un algoritmo di classificazione\index{classificazione!\acsfont{CTNBC}} che, dato in input il modello di un classificatore appreso e una istanza da classificare, inferisca la classe a cui è più probabile che tale istanza appartenga.
...
Di seguito si presenta il sorgente che, selezionata un'istanza dal \ds{2} (\ie{} il primo elemento nel dataset), apprende un classificatore\index{apprendimento!\acsfont{CTBNC}} \acs{CTNB} sui restanti elementi presenti del dataset (\autoref{lst:learn-ctbnc-infer1}) e utilizza tale modello per classificare la succitata istanza (\autoref{lst:infer-ctbnc-infer1}).
...
Come detto, utilizzando l'apposita funzione (si veda \autoref{lst:learn-ctbnc-infer2}) di apprendimento\index{apprendimento!\acsfont{CTNBC}} dei classificatori \acs{CTNB} (\acs{CTNBC}) è possibile ridurre la quantità di linee di codice e di operazioni necessarie, come mostra il seguente sorgente.

\vspace*{8pt}\inputsourcecode[caption={[Classificazione di un'istanza (\ds{2})]Apprendimento di un classificatore \acs{CTNB} (\acs{CTNBC}) tramite la apposita funzione e classificazione\index{classificazione!\acsfont{CTNBC}} di un'istanza del \ds{2} tramite tale classificatore.},label=lst:rctbn-infer-ctnbc,language=rstats]{codes/inferctnbc2}

L'output della funzione \lstinline[language=rstats]{infer_ctbnc}\index{inferenza} consiste in una lista etichettata composta due elementi: \lstinline$logp$, contenente un vettore etichettato delle \emph{log-probabilità} di ogni classe e \lstinline$prob$, contenente un vettore etichettato delle \emph{probabilità} assegnate ad ogni classe del modello \acs{CTNBC} di riferimento.
...
Lo scopo di questa sottosezione è presentare brevemente l'esecuzione del processo di \emph{\index{apprendimento!strutturale}} tramite il \pacchettor{}.

Nello specifico, dato un ipotetico \emph{training set}, si presenta l'utilizzo della funzione \lstinline[language=rstats]{greedy_search} per l'apprendimento del classificatore \acs{CTBN} (\acs{CTBNC}) che meglio rappresenta tali dati di addestramento\index{dati!addestramento, di|see{\trs{}}}, fissato il numero massimo di genitori di ogni nodo al valore $2$. Tuttavia si osservi che la funzione in questione può apprendere una qualsiasi struttura \acs{CTBN}. Tale funzione esegue l'\autoref{lst:hc-pseudo}, presentato \vpageref{lst:hc-pseudo}, per ogni nodo delle istanze del \emph{training set}, utilizzando come punteggio la funzione $famscore_{\conceptsym{B}}$ presentata nella \vref{sec:ctbn-structurallearning-score} (si veda l'\autoref{eq:bayesian-score}).
...
\end{itemize}
Tuttavia, l'unico parametro obbligatorio è il \emph{training set}, sul quale la procedura di \keyword{ricerca euristica} computa lo \emph{\keyword{score bayesiano}} al variare dei nodi e del relativo insieme di genitori. I restanti parametri sono tutti dotati di un parametro di default.

Il sorgente che segue illustra come espletare il compito prefissatoci.

\vspace*{8pt}\inputsourcecode[caption={[Apprendimento della struttura (\ds{2})]Apprendimento della struttura di un \acs{CTBN} (\acs{CTBNC}) da una porzione del \ds{2}.},label=lst:rctbn-hc,language=rstats]{codes/hc}

L'output che tale funzione restituisce è composto dalla struttura che meglio rappresenta i dati di addestramento\index{dati!addestramento, di|see{\trs{}}} e il relativo modello \acs{CTBN} computato in fase di ricerca.
...
Ottenuti i file di output di \acsfont{Sensors} \acs{DLL} è necessario trasformarli in un dataset\index{dataset!generazione} utile al framework delle \acs{CTBN}. A tale scopo sono stati sviluppati degli applicativi, brevemente discussi nella \vref{sec:dataset-tools}.

Il primo passo necessario consiste nella sostituzione dei \emph{time period\index{dataset!time period}} con le rispettive classi (si veda a tal riguardo la \vref{tab:ds-2-tp-labels}). A tale scopo è stato sviluppato un programma in linguaggio \lstinline[]|R| con interfaccia \lstinline[]|bash| chiamato \lstinline[]|sensors2dataset|.
...
\vspace*{8pt}\inputsourcecode[caption={[Sostituzione delle classi ai time period]Esecuzione del programma \lstinline[]|bash| per la sostituzione dei \emph{time period} con le relative classi nel file \acs{CSV} relativo al \ds{2}.},language=sh,basicstyle=\footnotesize\ttfamily,numbers=none,label=lst:sensors2dataset]{codes/sensors2dataset2}
...
\begin{itemize}
	...
	\item l'opzione \lstinline[]|r| comunica al programma di rimuovere la colonna relativa ai \emph{time period}
	\item l'opzione \lstinline[]|b| serve a definire i \emph{time period\index{dataset!time period}} a partire dai quali una nuova classe deve essere creata e associata
