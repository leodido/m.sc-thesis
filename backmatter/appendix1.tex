...
Di seguito si illustra il codice sorgente che, utilizzando tale funzione (si veda la \autoref{lst:learn-ctbnc-func-line}), apprende\index{apprendimento!\acsfont{CTNBC}} un \acs{CTBNC} da un \emph{\keyword{training set}} del \ds{2} (\ie{} da una porzione di elementi di tale dataset).
...
\vspace*{8pt}\inputsourcecode[caption={[Apprendimento di un \acs{CTBNC} (\ds{2})]Apprendimento di un classificatore \acs{CTBN}, la cui struttura viene generata in modo casuale, su un \emph{\keyword{training set}} facente parte del \ds{2}.},language=rstats,label=lst:rctbn-learc-ctbnc]{codes/learnctbnc}
...
Ottenuti i file di output di \acsfont{Sensors} \acs{DLL} è necessario trasformarli in un dataset\index{generazione!dataset} utile al framework delle \acs{CTBN}. A tale scopo sono stati sviluppati degli applicativi, brevemente discussi nella \vref{sec:dataset-tools}.
