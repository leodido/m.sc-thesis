...
P(\conceptsym{G})=\prod_{i} P(Pa(\setel{X}_i) = Pa_{\conceptsym{G}}(\setel{X}_i))\text{.}
\end{equation}
...
\end{equation}
Combinando l'assunzione di \emph{indipendenza dei parametri} discussa in \virgolette{\nameref{sec:ctbn-bayesian-estimate}} (\autoref{sec:ctbn-params}) con l'\autoref{eq:structurallearning-params-modularity} derivante dalla \emph{modularità dei parametri}, si ottiene:
\begin{equation}\label{eq:structurallearning-params-posterior}
\begin{split}
P(\param{q}_{\conceptsym{G}}\,,\param{$\theta$}_{\conceptsym{G}}\,\arrowvert\,\conceptsym{G})&=\prod_{\setel{X}_i}P\Big(\param{q}_{\setel{X}_i\,\arrowvert\,Pa(\setel{X}_i)}\,\arrowvert\,Pa(\setel{X}_i)=Pa_{\conceptsym{G}}(\setel{X}_i)\Big) \cdot \\
& \quad \cdot P\Big(\param{$\theta$}_{\setel{X}_i\,\arrowvert\,Pa(\setel{X}_i)}\,\arrowvert\,Pa(\setel{X}_i)=Pa_{\conceptsym{G}}(\setel{X}_i)\Big)\text{\normalsize.}
\end{split}
\end{equation}
Si osservi che, poiché $P(\conceptsym{G})$ è indipendente dalla quantità dei dati, il termine significativo dell'\autoref{eq:structurallearning-score} è la \emph{likelihood marginale}, $P(\conceptsym{D}\,\arrowvert\,\conceptsym{G})$. Tale termine, infatti, codifica l'incertezza sui parametri integrando su tutti i possibili valori che essi possono assumere:

\begin{equation}\label{eq:structurallearning-prob-data-posterior}
