...
Perciò possiamo ora calcolare la likelihood del dataset $\conceptsym{D}$ rispetto a $\setel{X}(t)$.
\begin{equation}
...
Perciò la likelihood dei dati $\conceptsym{D}$ rispetto a $\conceptsym{N}$ è il prodotto delle likelihood di ogni variabile $\setel{X_n}$:
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
Il termine $L_{\setel{X_i}}(\theta_{\setel{X_i}\,|\,\setel{u}}:\conceptsym{D})$ è la probabilità di una sequenza di transizioni trascurando il tempo che intercorre fra esse poiché esse dipendono esclusivamente dal valore di nodi genitori. Usando le \emph{sufficient statistics} possiamo perciò scrivere:
\[
L_{\setel{X_i}}(\theta_{\setel{X_i}\,|\,\setel{u}}:\conceptsym{D}) = \prod_{u} \prod_{x} \prod_{x\neq x^{\prime}}\theta_{xx^{\prime}\,|\,u}^{\mstat[x]{x^{\prime}\,|\,u}}
\]
