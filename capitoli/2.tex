...
Introdotte le \emph{sufficient statistics} si approccia ora il problema dell'apprendimento dei parametri di una \acl{CTBN} (con struttura nota) da un insieme di dati completi.
I parametri, infatti, sono funzione delle \emph{sufficient statistics}.



qx|u = M[x|u]/T[x|u]
thetax,x'|u = M[x,x'|u]/M[x|u]
Relazione fra i parametri: thetaij = qij / qi
oss.: sono maximum-likelihood parameters (MLE) poiché pensati per
massimizzare la probabilità a posteriori di un dataset, dato un modello
(dataset likelihood)
