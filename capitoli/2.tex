...
Data quindi una matrice di intensità $\imsign{\setel{X}}$ essa descrive il comportamento transiente di $\setel{X}(t)$. Se $\setel{X}(0)=x_i$ allora il processo di Markov omogeneo (e indicizzato dal tempo $t$) $\setel{X}(t)$ rimarrà nello stato $x_i$ una quantità di tempo esponenzialmente distribuita rispetto al parametro $q_{x_i}$. Di conseguenza la funzione di densità di probabilità $f$ e la corrispondente funzione di distribuzione $F$ per $\setel{X}(t)$ che rimane uguale a $x_i$ sono date da:
\begin{equation}
\begin{split}
...
Si osservi infine come la matrice $\imsign{\setel{X}}$ fa in modo che $\setel{X}$ soddisfi la proprietà di Markov poiché il comportamento futuro di $\setel{X}$ è definito solamente in base al suo stato attuale (\eqref[perciò vale l'equazione]{eq:homogeneus-markov-process}).7
...
\end{itemize}
 ...
 \`E ora possibile quindi presentare le \acl{CTBN} come una collezione di \ac{CTMP} con spazio degli stati discreto~\cite{Nodelman2007}.
...
Si noti comunque che si affronta solamente il processo di apprendimento da dati completi mentre quello relativo a dati non completi viene tralasciato poiché non è argomento di questo lavoro di tesi.

Per dati completi si intende un insieme $\conceptsym{D}=\{\,\delta_1\,,\,\dotsc\,,\,\delta_h\,\}$ composto da una o più traiettorie di tutte le variabili casuali di una \acs{CTBN}, dove ogni $\delta_i$ è un insieme completo di transizioni fra stati (indicizzato dal tempo): per ogni momento temporale di ogni traiettoria conosciamo l'istanziazione di tutte le variabili casuali in questione.

\subsection{Sufficient Statistics}
% TODO: parte sulla likelihood? Nodelmann2007 4.2
Lo scopo delle \emph{sufficient statistics} è riassumere la dinamica evolutiva di un singolo \homm\mprocess{} $\setel{X}(t)$ tramite:
\begin{itemize}
...
\]
Traslando il concetto alle \acl{CTBN} e quindi ai \conm\mprocess{} è necessario considerare le succitate quantità contestualmente all'istanziazione dell'insieme $\set{U}$ dei nodi genitori. Poiché, nel caso di dati completi, si conosce l'istanziazione di $\set{U}$ durante l'intera traiettoria allora, per ogni preciso momento nel tempo $t$, si conosce quale matrice di intensità (omogenea) $\cimsign{\setel{X}}{\setel{u}}$ governi la dinamica di $X$.
Perciò otteniamo:
\begin{itemize}
...
\end{itemize}
% TODO: parte sulla likelihood? Nodelmann2007 4.5

% TODO: come calcolare una CIM dalle SS -> 5.1

\subsection{Stima dei parametri}
Introdotte le \emph{sufficient statistics} possiamo approcciare il problema dell'apprendimento dei parametri di una \acl{CTBN} di cui conosciamo la struttura da un insieme di dati completi.
...
% Per ogni sottoinsieme delle variabili casuali $\setel{X_n}$ (con $n=1,\dotsc,N$), il $J$-evidence-stream è l'insieme delle istanziazioni comuni $\setel{X}=x$ associate ad ogni $J$-time-segment. \`E denotato con $(\setel{X^1}=x^1,\,\setel{X^2}=x^2,\,\dotsc,\,\setel{X^J}=x^J)$, o più concisamente $(x^1,\,x^2,\,\dotsc,\,x^J)$.
% \end{definizione}
