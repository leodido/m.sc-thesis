...
\end{definizione}
\begin{nota}
\`E possibile riferirsi a tale concetto anche tramite l'espressione \emph{\qmarks{insieme dei segmenti temporali}}.
\end{nota}
\begin{definizione}[$J$-evidence-stream]\label{defn:j-evidence-stream}
Dato un $J$-time-segment composto da $J$ intervalli temporali e una \pv{} $\set{X}$ composta da $N$ variabili casuali, un $J$-evidence-stream è l'insieme delle istanziazioni comuni $\set{X}=\set{x}$ associate ad ogni $J$-time-segment per ogni sottoinsieme delle variabili casuali~\citep{Stella2012}. \`E denotato con $(\set{X}^1=\set{x}^1\,,\,\set{X}^2=\set{x}^2\,,\,\dotsc\,,\,\set{X}^J=\set{x}^J)$, o più concisamente $(\set{x}^1\,,\,\set{x}^2\,,\,\dotsc\,,\,\set{x}^J)$.
\end{definizione}
Un $J$-evidence-stream $(\set{x}^1\,,\,\set{x}^2\,,\,\dotsc\,,\,\set{x}^J)$ è detto essere completamente osservato (\ie{} \emph{fully observed}) se lo stato di tutte le variabili $\setel{X_n}\in\set{X}$ è conosciuto in tutto l'intervallo $[\,0,\,T)$. Viceversa, un $J$-evidence-stream è detto parzialmente osservato (\ie{} \emph{partially observed}).
