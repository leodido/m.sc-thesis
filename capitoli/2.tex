...
Una \acs{BN} rappresenta la distribuzione di probabilità congiunta del suo insieme di variabili casuali tramite la distribuzione di probabilità condizionale di ognuna di essa (si veda \eqref[l'equazione]{eq:bn-chain-rule}).
...
Si noti che il concetto appena espresso gioca un ruolo importante per i modelli probabilistici, quali sono le \acl{BN}, semplificando i calcoli richiesti per l'inferenza e l'apprendimento. Le \acl{BN} ereditano questi benefici dell'indipendenza condizionale come conseguenza della loro definizione (si veda \myref[l'osservazione]{oss:bn-markov-assumption}). Infatti, la distribuzione condizionale di ogni variabile casuale $\setel{X_i}$ dipende solo ed esclusivamente dal valore dei suoi genitori, $pa(\setel{X_i})$, mentre ignora completamente i valori dei nodi che non discendono da essa, $nd(\setel{X_i})$.
...
\label{defn:cim}
Dato un insieme di processi di Markov $\set{U}$, una \im*{} condizionale $\cimsign{\setel{X}}{\set{U}}$ è costituita da un insieme di matrici di intensità $\cimsign{\setel{X}}{\setel{u_i}}$, una per ogni diversa istanziazione $\setel{u_i}$ di $\set{U}$.
\[
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item una \im*{} condizionale $\cimsign{\setel{X_n}}{pa(\setel{X_n})}$ per ogni nodo $\setel{X_n} \in \set{X}$.
    \end{itemize}
\end{itemize}
\end{definizione}
