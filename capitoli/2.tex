...
\label{defn:ctbn}
Data una Process Variable $\set{X}$, insieme di processi di Markov $\setel{X_1}\,,\,\setel{X_2}\,,\,\dotsc\,,\,\setel{X_N}$ a tempo continuo e con spazio degli stati finito $val(\setel{X_n})=\{\,\vectel{x_1}\,,\,\dotsc\,,\,\vectel{x_J}\,\}$ (dove $n=1\,,\,\dotsc\,,\,N$), una \acs{CTBN} $\conceptsym{N}$ su $\set{X}$ consiste di:
\begin{itemize}
...
Poiché tutte le transizioni sono osservabili la probabilità del dataset $\conceptsym{D}$ può essere decomposta come un prodotto delle probabilità individuali di ogni singola transizione $d$. Data quindi una tripla $d=\langle \,x_{d}\,,\,t_d\,,\,x_d^{\prime}\, \rangle\in\conceptsym{D}$ che esprime una transizione di $\setel{X}(t)$ da $x_d$ a $x_d^{\prime}$ dopo essere rimasto $t_d$ tempo in $x_d$, possiamo scrivere la probabilità di questa singola transizione $d$ in funzione dei parametri (\ref{sec:ctbn-params}):
\begin{equation}
\begin{split} 
L_{\setel{X}}(q\,,\,\theta\,|\,d) &= L_{\setel{X}}(q\,|\,d)\,L_{\setel{X}}(\theta\,|\,d) \\
&= q_{x_d}\,exp(-q_{x_d}\,t_d)\,(\theta_{x_{d}x_d^{\prime}})
\end{split}
\end{equation}

\eqref[dall'equazione]{eq:im-distrib} e \autoref{defn:im} ...
% TODO: parte sulla likelihood? Nodelmann2007 4.2

Le \emph{sufficient statistics} per un singolo \homm\mprocess{} $\setel{X}(t)$ riassumono la sua dinamica evolutiva:
\begin{itemize}
...
\]
Traslando il concetto alle \acl{CTBN} e quindi ai \conm\mprocess{} è necessario considerare le succitate quantità contestualmente all'istanziazione dell'insieme $\set{U}$ dei nodi genitori. Poiché, nel caso di dati completi, si conosce sempre l'istanziazione di $\set{U}$, allora, per ogni preciso momento nel tempo $t$, si conosce quale matrice di intensità (omogenea) $\cimsign{\setel{X}}{\setel{u}}$ governi la dinamica di $X$.
% TODO: hmm ... manca un "collegamento"
Perciò otteniamo:
\begin{itemize}
