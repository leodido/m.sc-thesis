...
Sempre al fine di preparare la discussione delle \acl{CTBN} si prosegue presentando alcuni concetti propedeutici relativi ai Markov process, una categoria di processi stocastici \emph{memoryless} (con assenza di memoria).
...
Dalla proprietà di Markov deriva la definizione di Markov Process.
...
\begin{definizione}[\omprocess{} Process]
...
% TODO: dire qua che induce una distribuzione di probabilità fattorizzata .. ? (vedi slide mie)
\end{definizione}
...
\begin{itemize}
    ...
    \item la \emph{probabilità di transizione} dallo stato $x_i$ allo stato $x_j$ \[\theta_{ij}=q_{x_{ij}}/q_{x_i}\]
\end{itemize}

Si osservi come la matrice $\imsign{\setel{X}}$ fa in modo che $\setel{X}$ soddisfi la proprietà di Markov.

\begin{definizione}[\cmprocess{} Process]
\label{defn:conditional-markov-provess}
Un processo di Markov le cui intensità di transizione variano nel tempo non in funzione del tempo ma in funzione dei valori assunti ad ogni determinato istante $t$ da un insieme di altre variabili, che evolvono anch'esse come dei processi di Markov, è detto essere un \cmprocess{} Process (o Inhomogeneous Markov Process).
\end{definizione}
...
Se specifichiamo una distribuzione di probabilità iniziale su $\setel{X}$ abbiamo così definito un Markov Process il cui comportamento dipende dall'istanziazione dei valori di $\setel{U_i}$. 
...
\[
\cimsign{\setel{X}}{\set{U}}=\big\{\,\cimsign{\setel{X}}{\setel{u_1}}\,,\,\cimsign{\setel{X}}{\setel{u_2}}\,,\,\dotsc\,,\,\cimsign{\setel{X}}{\setel{u_m}}\,\big\}
\]
\[
\cimsign{\setel{X}}{u_i} = \begin{bmatrix}
                        -q_{x_1}    & q_{x_1x_2}& \cdots & q_{x_1x_k}   \\[0.5em]
                        q_{x_2x_1}  & -q_{x_2}  & \cdots & q_{x_2x_k}   \\[0.5em]
                        \vdots      & \vdots    & \ddots & \vdots       \\[0.5em]
                        q_{x_kx_1}  & q_{x_kx_2}& \cdots & -q_{x_k}
...
\subsection{\acl{CTMP}}
\label{sec:ctmp}
Avendo brevemente introdotto i concetti basilari relativi ai processi di Markov si fornisce in questa sezione uno sguardo su una loro estensione, i processi di Markov a tempo continuo (e stati finiti).

Con il termine \acf{CTMP} si intende indicare una variabile casuale indicizzata dal tempo. Questo concetto, come sarà evidente ben presto nel prosieguo, è parte fondamentale e imprescindibile delle \acl{CTBN}.
\acresetall
\begin{definizione}[Variabile di processo]
Dato un insieme di variabili casuali $\set{X}$ indicizzate dal tempo $t\in[\,0,\,\infty)$ è conveniente definire questo insieme come una singola \emph{Process Variable} $\set{X}(t)$.
\end{definizione}
L'istanziazione di un insieme di valori per $\set{X}(t)$ al variare di $t$ è detta \emph{traiettoria}.

VP -> markov process omogeneo tempo continuo stati finiti
singola V -> markov process condizionale (o inomogeneo), tempo continuo stati finiti
...
Una \acl{CTBN} è un modello grafico in cui ogni nodo è associato con una variabile casuale i cui stati evolvono nel tempo continuo. Le dinamiche evolutive degli stati dei nodi sono governate e dipendano dal valore che gli stati dei nodi padre\footnote{Con il termine <<nodo padre>>, o \emph{parent node}, si intende un nodo il cui stato condiziona quello di un altro nodo del modello grafico.} assumono~\cite{Stella2012}.
...
Data una Process Variable $\set{X}$, cioè un insieme di variabili casuali $\setel{X_1}, \setel{X_2}, \dotsc, \setel{X_N}$ dove ogni $\setel{X_n}$ ha uno spazio degli stati finito $val(\setel{X_n})=\{\vectel{x_1}, \dotsc, \vectel{x_J}\}$, una \acs{CTBN} $\conceptsym{N}$ su $\set{X}$ consiste di:
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item una matrice di intensità condizionale $\cimsign{\setel{X_n}}{pa(\setel{X})}$ per ogni nodo $\setel{X_n} \in \set{X}$.
    \end{itemize}
\end{itemize}
\end{definizione}
\begin{nota}
...
Per ogni sottoinsieme delle variabili casuali $\setel{X_n}$ (con $n=1,\dotsc,N$), il $J$-evidence-stream è l'insieme delle insieme delle istanziazioni comuni $\setel{X}=x$ associate ad ogni $J$-time-segment. \`E denotato con $(\setel{X^1}=x^1,\,\setel{X^2}=x^2,\,\dotsc,\,\setel{X^J}=x^J)$, o più concisamente $(x^1,\,x^2,\,\dotsc,\,x^J)$.
\end{definizione}

\begin{osservazione}
Un $J$-evidence-stream $(x^1,\,x^2,\,\dotsc,\,x^J)$ è detto essere \emph{<<fully observed>>} (completamente osservato) se lo stato di tutte le variabili $\setel{X_n}$ è conosciuto in tutto l'intervallo $[\,0,\,T)$. Viceversa, un $J$-evidence-stream è detto \emph{<<partially observed>>} (parzialmente osservato)~\cite{Stella2012}.
\end{osservazione}
