...
Il problema dell'\emph{apprendimento \keywordsub[apprendimento]{struttura}le} da \emph{\keyword{dati completi}} di una \acl{CTBN} (\acs{CTBN}) è quindi l'argomento trattato in questo capitolo.
...
L'approccio che si presenta in questo capitolo è quindi un approccio basato sul \keywordsub[apprendimento]{punteggio}: si definisce una funzione che computa uno \emph{\keyword{score bayesiano}} finalizzato alla valutazione di ogni struttura rispetto ai \keyword{dati di addestramento} (\ie{} \emph{\keyword{\trs{}}}) e si usa una tecnica di \keyword{ricerca euristica} (\eg{} la ricerca \emph{\keyword{\hc{}}}) per cercare nello spazio delle strutture candidate quella che esibisce il maggior punteggio.
...
L'obiettivo di questa sezione è quindi presentare una funzione di scoring per l'apprendimento strutturale delle \acl{CTBN} (\acs{CTBN}). Lo scopo di tale funzione è calcolare il \keyword{punteggio} (\ie{} lo \emph{score}) di una \keywordsub[apprendimento]{struttura} relativamente al \emph{\keywordsub[apprendimento]{\trs{}}} $\conceptsym{D}$ fornito.
...
In questa sezione si affronta il secondo passo del processo di apprendimento strutturale: l'utilizzo di una \emph{procedura di \keywordsub[apprendimento]{ottimizzazione}} finalizzata alla ricerca di una struttura che massimizzi lo \emph{\keywordsub[apprendimento]{score bayesiano}}.
...
Si osservi che, fissando $k$ a priori, non è necessario enumerare esaustivamente tutti i possibili insiemi di nodi genitori di ogni nodo di una \acs{CTBN}. Di conseguenza è possibile utilizzare un algoritmo di ricerca euristica di tipo \emph{\keywordsub[apprendimento]{greedy}} per esplorare lo spazio di ricerca. Nel seguito si presenta perciò l'algoritmo utilizzato, il cui nome è \emph{\keyword{\hc{}}}.

\subsection{Hill Climbing}\label{subsec:structurallearning-hc}
L'algoritmo \emph{\keywordpagb{\hc{}}}



%listato
%definizione generale hill climbing
%commento
