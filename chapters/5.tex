...
\subsection{Caratteristiche}\label{sec:tsis-features}
...
\begin{enumerate}
    ...
    \item a~\autoref{lst:netsim-dets-id-import} si importa la lista dinamica dei numeri identificativi assegnati dall'utente ai sensori, \lstinline[]|*NETSIM_DETECTORS_mp_DETID|; rinominata, a~\autoref{lst:netsim-dets-id-assign}, in \lstinline[]|net_det_id|
...
DLL_IMPORT int *NETSIM_DETECTORS_mp_DETID;<ls>\label{lst:netsim-dets-id-import}<le>
#define net_det_id NETSIM_DETECTORS_mp_DETID<ls>\label{lst:netsim-dets-id-assign}<le>
...
L'obiettivo ultimo di \acsfont{Sensors} \acs{DLL} consiste nella generazione di un file \acs{CSV}\footnote{\acf{CSV} è un formato basato su file di testo utilizzato per l'importazione ed esportazione (ad esempio da fogli elettronici o database) di una tabella di dati.} che rappresenti il passaggio dei veicoli nel tempo sui vari sensori presenti nella rete stradale. Il monitoraggio dei sensori deve essere effettuato con la massima granularità temporale possibile (\ie{}, $0.1$ secondi), anche nel caso di reti stradali urbane.

%leggere file di input ricreando la rete che esso rappresenta
%utilizzare campo di 10 bit
%quali punti di chiamata CORSIM utilizzati e a quale scopo?
...
\subsection{Formato dell'output}
...
1.10000,1,0,1,0,0,0,1
1.20000,1,0,1,0,0,0,1
1.30000,1,0,1,0,0,0,0
1.40000,1,0,0,0,0,0,0
1.50000,1,0,0,0,0,0,0
\end{lstlisting}

La~\vref{tab:sensors-output-semantic} illustra il significato di ogni colonna dei dati tabulari restituiti da \acsfont{Sensors} \acs{DLL}.
\begin{table}[H]%
\begin{tabularx}{\columnwidth}{+l^X}
\toprule\rowstyle{\bfseries}%
...
identificatore sensore  & $1$ in caso di passaggio di un veicolo sul rispettivo sensore, $0$ altrimenti.    \\\bottomrule
\end{tabularx}
...
% TODO: parlare di cosa si intende ottenere!
...
% TODO: collegarsi .. dall'output di sensors vogliamo ottenere ..
% ottimizzazioni (rimozione linee duplicate -> rappresentare solo le transizioni quando avvengono)
