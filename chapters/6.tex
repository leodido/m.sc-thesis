...
Ottenuti i risultati relativi ai processi di classificazione, per ognuno dei $10$ \emph{\keyword{fold}} si è effettuato il processo di \emph{\keyword{cross-tabulazione}}\footnote{Il processo di \keyword{cross-tabulazione} consiste in un processo statistico il cui scopo è riassumere le interrelazioni che intercorrono fra due variabili categoriche dei dati. L'output di questo processo corrisponde solitamente a una \emph{\keyword{matrice di confusione}}.} tra le classi reali delle istanze e le classi ad esse assegnate dal processo di \emph{classificazione supervisionata\index{classificazione!supervisionata}}. Calcolate così le \emph{matrici di confusione\index{matrice!confusione,di}}, si è proceduto alla computazione delle metriche di valutazione per ogni \emph{\keyword{fold}} e successivamente all'effettuazione delle medie \emph{\keyword{macro-average}} e \emph{\keyword{micro-average}}.
...
\begin{table}[ht]
    ...
    \end{tabular}
    \caption[Matrice di confusione\index{matrice di confusione} multi-classe]{Esempio di \emph{\keyword{matrice di confusione}} specifico del problema di \emph{classificazione multi-classe\index{classificazione!multi-classe}} affrontato.}\label{tab:contingency-table}
\end{table}
\normalsize
