...
A partire dalla \vref{tab:contingency-table}, per ognua delle classi, è possibile calcolare una ulteriore \emph{\keyword{matrice di contingenza}} che riassumi la classificazione di ogni singola classe. La \vref{tab:contingency-table-single-class} di seguito riporta un esempio di tale matrice. Fissata una classe, tale tabella, tramite alcune metriche di base che si descrivono di seguito, mette in relazione le classi reali delle istanze con le classi inferite.
\begin{table}[ht]
	\centering
...
\begin{description}
	...
	Il \acf{TNR}, detto anche \emph{\keyword{specificity}}, misura la proporzione di istanze correttamente classificate come non appartenenti a quella reale in esame. \`E così calcolato:\par
	\[TNR=\frac{TN}{TN + FP} = 1 - FNR\text{.}\]
	\item[False Negative Rate] \hfill \\
	Il \acf{FNR} è l'inverso della metrica \acs{TNR}. \`E quindi calcolato nel seguento modo:\par
	\[FNR = \frac{FP}{TN + FP} = 1 - TNR\text{.}\]
	\item[Positive Predictive Value] \hfill \\
	Il \acf{PPV}, detto anche \emph{\keyword{precision}},
	\item[False Discovery Rate] \hfill \\
	Il \acf{FDR} ...
	...
	L'\emph{\keyword{accuracy}}
\end{description}

%proceduto alla creazione delle relative \emph{matrici di contingenza}\footnote{Una \keyword{matrice di contingenza}} e quindi computazione delle seguenti metriche di valutazione:
%\begin{itemize}
%	\item
%\end{itemize}

%Per ognungo dei fold della $k$-\emph{fold \keyword{cross-validation}} si è computata la metrica \emph{\keyword{accuracy}} con intr
...
In questa sottosezione si presentano i risultati di ogni processo di \emph{\keyword{classificazione supervisionata}} effettuato con ognuno $4$ modelli di classificatori precedentemente descritti. Nello specifico, per ogni dataset sottoposto a sperimentazione, si illustra il valore della metrica \emph{\keyword{accuracy}} relativo ad ognuno dei classificatori utilizzati.
...
%comparazione accuracy:
%%def. accuracy
%%tabella accuracy corrispondente al grafico
%%6 dataset -> 4 modelli per ognuno
%%comparazione intra dataset
%%comparazione inter dataset
%%incremento per traiettorie più lunghe (probabilmente perché perde meno transizioni nel taglio ..)

\subsection{Curve ROC}
%comparazione curve ROC per classe
%%def. ROC
%%tabella AUC?
%%citare paper daniele e fawcett
%... vedi txt
