...
\begin{description}
	...
	L'accuratezza di un modello (\ie{} \emph{accuracy\index{metriche!accuracy}}), intesa come la sua capacità di classificare correttamente tutte le istanze (che appartengano o meno alla classe in esame), è computata nel seguente modo:
...
In questa sottosezione si presentano i risultati dei processi di \emph{\keyword{classificazione supervisionata}} dei $6$ dataset effettuati per ognuno dei $4$ modelli di classificatori presi in considerazione. Per ogni dataset sottoposto a sperimentazione si illustra e commenta il valore della metrica \emph{accuracy\index{metriche!accuracy}} relativo ad ognuno dei classificatori utilizzati. Si rimarca che la metrica \emph{accuracy\index{metriche!accuracy}} è stata calcolata tramite approccio \emph{\keyword{micro-average}} su tutti i \emph{\keyword{fold}} di ogni sperimentazione.
\cleardoublepage
\begin{table}
	\centering
	...
	\end{tabular}
	\caption[Accuratezza dei classificatori \acs{CTBN} e \acs{CTNB}]{Comparazione del valore di \emph{accuracy\index{metriche!accuracy}} ottenuto dal classificatore \acs{CTNB} e dai classificatori \acs{CTBN} (appresi con numero massimo di genitori variabile da $2$ a $4$) su ognuno dei $6$ dataset generati. In rosso i valori maggiori per ogni dataset.}\label{tab:accuracies-table}
\end{table}

\begin{figure}
	\centering
	\includegraphics[width=1.3\textwidth,center]{accuracies-by-ds-by-classifier-green.pdf}
	\caption[Accuratezza dei classificatori \acs{CTBN} e \acs{CTNB}]{Comparazione del valore di \emph{accuracy\index{metriche!accuracy}} ottenuto dal classificatore \acs{CTNB} e dai classificatori \acs{CTBN} (appresi con numero massimo di genitori variabile da $2$ a $4$) su ognuno dei $6$ dataset generati.}\label{fig:accuracies-fig}
\end{figure}
%\begin{figure}
%	\centering
...
Questa metrica, inizialmente utilizzata in altri campi di ricerca (\eg{} teoria delle decisioni), è sempre più frequentemente utilizzata anche nel campo dell'apprendimento automatico. Ciò poiché l'utilizzo della sola \emph{accuracy\index{metriche!accuracy}} come metrica per la valutazione dell'efficacia dei classificatori si rivela spesso carente dal punto di vista informativo e perciò inadeguato \citep[per approfondimenti si veda][]{Provost1997,Provost2001}.
