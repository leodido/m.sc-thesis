...
Mentre nella \autoref{sec:learning-ctbnc} si affronta il processo di \emph{\keyword{apprendimento}} in caso di \emph{dati completi} dei \acs{CTBNC}, nella \autoref{sec:inference-ctbnc}, si presenta un algoritmo di \emph{\keyword{inferenza} esatta} per la classe dei \acs{CTBNC}.
...
function ctnbclearn(data, classes, graph) {
    ...
    var nk = len(class)
    var priors[nk]
    for (i in keyword(data)) {
        var k = keyword(classes[i])
...
function learn(data, classes, graph) {
    ...
    var m(), t()
    foreach (i in keyword(data)) {
        var j = 1
        while (<ls>$t_j \leq T_i$<le>) {
            foreach (n in keyword(graph.nodes)) {
...
\item la likelihood del flusso di evidenze dato il valore della variabile classe, a cui ci si riferisce nel prosieguo usando l'espressione \emph{\virgolette{\keyword{likelihood temporale}}} \[P(\evidencestream{J}\,\arrowvert\,\setel{Y})\text{.}\]
\end{itemize}
...
\end{equation}
Il primo termine dell'\autoref{eq:ctbnc-inference-2}, cioè la \emph{\keyword{likelihood temporale}}, è invece fondamentale per la classificazione tramite regola \acs{MAP} ed è possibile riformularlo nel seguente modo:
...
dove:
\begin{itemize}
    \item $P(\set{x}^j\,\arrowvert\,\setel{Y})$ rappresenta la probabilità che il \emph{\keyword{vettore aleatorio}}\footnote{\label{note:vettore-aleatorio}Un \emph{vettore aleatorio} $\set{X}=(\setel{X}_1,\setel{X}_2,\,\dotsc\,,\setel{X}_n)$ è una $n$\emph{-upla} (composta da $n$ variabili casuali) i cui elementi sono dati da numeri aleatori.} $\set{X}$ resti nello stato $\set{x}^j$ durante l'intervallo temporale $[\,t_{j-1},\,t_j)$ data evidenza sulla variabile classe $\setel{Y}$
...
function infer(ctbnc, timeseg, stream) {
    ...
    var logp[len(priors)]
    for (k in keyword(priors)) {
        logp[k] = log(priors[k])
    <ls>\label{lst:inference-priors-end}<le>}
    <ls>\label{lst:inference-classes-for}<le>for (k in keyword(priors)) {
        <ls>\label{lst:inference-stream-for}<le>for (j in keyword(timeseg)) {
            <ls>\label{lst:inference-nodes-for}<le>for (n in keyword(ctbnc.graph.nodes)) {
